...
class Payment {
  constructor({appid, mchid, partnerKey, pfx, notify_url, refund_url, spbill_create_ip, sandbox} = {}, debug = false) {
    ...
    this.urls = sandbox ? {
      ...
      unifiedorder: 'https://api.mch.weixin.qq.com/sandboxnew/pay/unifiedorder', // 统一下单
      orderquery: 'https://api.mch.weixin.qq.com/sandboxnew/pay/orderquery', // 查询订单
      closeorder: 'https://api.mch.weixin.qq.com/sandboxnew/pay/closeorder', // 关闭订单
      refund: 'https://api.mch.weixin.qq.com/sandboxnew/secapi/pay/refund', // 申请退款
      refundquery: 'https://api.mch.weixin.qq.com/sandboxnew/pay/refundquery', // 查询退款
      downloadbill: 'https://api.mch.weixin.qq.com/sandboxnew/pay/downloadbill', // 下载交易账单
      downloadfundflow: 'https://api.mch.weixin.qq.com/sandboxnew/pay/downloadfundflow', // 下载资金账单
  ...
  async _parse(xml, type, signType) {
    ...
    switch (type) {
      case 'middleware_nativePay':
        break;
      case 'unifiedorder':
        if (json.return_code !== 'SUCCESS') throw new Error(json.retmsg || 'XMLDataError');
        break;
      default:
  ...
  middleware(type = 'pay') {
    return async (ctx, next) => {
      ...
      try {
        if (typeof ctx.request.body !== 'string') throw new Error('XMLDataError_koa');
